...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagMsg_Control)
    {
        info->event[info->num] = EVENT_Msg_Control;
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valueMsg_Control,issuedData->attrVals.valueMsg_Control,sizeof(issuedData->attrVals.valueMsg_Control));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(0 != memcmp((uint8_t *)&last->valueMsg_Control,(uint8_t *)&cur->valueMsg_Control,sizeof(last->valueMsg_Control)))
    {
        GIZWITS_LOG("valueMsg_Control Changed\n");
        ret = 1;
    }

    if(0 != memcmp((uint8_t *)&last->valueMsg_Reply,(uint8_t *)&cur->valueMsg_Reply,sizeof(last->valueMsg_Reply)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMsg_Reply Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizMemcpy((uint8_t *)devStatusPtr->valueMsg_Control,(uint8_t *)&dataPoints->valueMsg_Control,sizeof(dataPoints->valueMsg_Control));
    gizMemcpy((uint8_t *)devStatusPtr->valueMsg_Reply,(uint8_t *)&dataPoints->valueMsg_Reply,sizeof(dataPoints->valueMsg_Reply));
    return 0;
}
